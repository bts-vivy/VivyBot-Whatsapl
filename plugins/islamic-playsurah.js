...
const handler = async (m, { args, conn, usedPrefix, command }) => {
    if (!args[0]) {
        const listText = daftarSurah.slice(0, 114).map(surah => `• ${surah}`).join('\n');
        return m.reply(`Masukkan nomor surah untuk mendengarkan audio.\n\nContoh: *${usedPrefix + command} 1*\n\n*Daftar Surah:*\n${listText}`);
    }

    let nomor = parseInt(args[0]);
    if (isNaN(nomor) || nomor < 1 || nomor > 114) return m.reply('Nomor surah tidak valid! Harus dari 1 sampai 114.');

    let url = `https://api.neoxr.eu/api/quran-audio?surah=${nomor}&apikey=${neoxr}`;
    m.reply('Sedang mengambil audio, mohon tunggu...');

    try {
        let res = await fetch(url);
        let json = await res.json();
        if (!json.data?.audio) return m.reply('Gagal mengambil audio.');
...
handler.help = ['playsurah <nomor surah>'];
handler.tags = ['islamic'];
handler.command = /^playsurah$/i;
handler.limit = true;
handler.daftar = true
