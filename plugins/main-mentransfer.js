            ...
            case 'koin':
                if (global.db.data.users[m.sender].koin >= count * 1) {
                    try {
                        ...
                        conn.reply(m.chat, `Berhasil transfer Koin Sebesar ${count}`.trim(), m)
                    } catch (e) {
                        global.db.data.users[m.sender].koin += count * 1
                        m.reply('Gagal Menstransfer')
                        console.log(e)
                    }
                } else conn.reply(m.chat, `Koin Kamu Tidak Mencukupi Untuk transfer Sebesar ${count}`.trim(), m)
                break
            case 'limit':
                if (global.db.data.users[m.sender].limit >= count * 1) {
                    try {
                        ...
                        conn.reply(m.chat, `Berhasil transfer Limit Sebesar ${count}`.trim(), m)
                    } catch (e) {
                        global.db.data.users[m.sender].limit += count * 1
                        m.reply('Gagal Menstransfer')
                        console.log(e)
                       }
                } else conn.reply(m.chat, `Limit Kamu Tidak Mencukupi Untuk transfer Sebesar ${count}`.trim(), m)
                break
            default:
                return conn.reply(m.chat, `Gunakan Format .transfer <type> <jumlah> <@tag>\nContoh Penggunaan: *.transfer koin 100 @tag*\n\n*List Yang Bisa Di Transfer :*\n- Koin\n- Limit`.trim(), m)
        }
    } catch (e) {
        conn.reply(m.chat, `Gunakan Format .transfer <type> <jumlah> <@tag>\nContoh Penggunaan: *.transfer koin 100 @tag*\n\n*List Yang Bisa Di Transfer :*\n- Koin\n- Limit`.trim(), m)
        console.log(e)
        }
}
